...
def trends(src_id, param):
    ...
    query on the Aylien API according to these parameters.
...
def timeSeries(src_id):
    """
    A function that takes the a list of query parameter values and runs a time
    series query on the Aylien API according to these parameters.
    """
    today = calc_day(0)
...
def main():
    ...
    os.system(bq_loc+" query --use_legacy_sql=false 'DELETE FROM `aylien-production`.customer_report.facts_category_example WHERE 1=1'")
    os.system(bq_loc+' load --source_format=CSV customer_report.facts_category_example ihs_cat_facts.csv')
    
    os.system(bq_loc+" query --use_legacy_sql=false 'DELETE FROM `aylien-production`.customer_report.facts_language_example WHERE 1=1'")
    os.system(bq_loc+' load --source_format=CSV customer_report.facts_language_example ihs_lang_facts.csv')

    os.system(bq_loc+" query --use_legacy_sql=false 'DELETE FROM `aylien-production`.customer_report.facts_stories_example WHERE 1=1'")
    os.system(bq_loc+' load --source_format=CSV customer_report.facts_stories_example ihs_story_counts.csv')
