...
def trends(src_id, param):
    ...
    headers = {'X-AYLIEN-NewsAPI-Application-ID': 'ihs-orange',
               'X-AYLIEN-NewsAPI-Application-Key': 'vtMNEfk3NuqcRzEt'
              }
    ...
    url = "https://api.ihs.orange.gcp.aylien.com/api/v1/trends"
    timeout_flag = 0
...
def timeSeries(src_id):
    """
    A function that takes the a source id and runs a time
    ...
    url = "https://api.ihs.orange.gcp.aylien.com/api/v1/time_series"
    response = requests.get(url=url, headers=headers, params=parameters)
    if response.status_code != 200:
        status = str(response.status_code)
        if response.status_code == 429:
...
def main():
    ...
    cat_frame_final.to_csv('ihs_orange_cat_facts.csv', header=None, index=None)
    ...
    lang_frame.to_csv('ihs_orange_lang_facts.csv', header=None, index=None)
    ...
    story_frame.to_csv('ihs_orange_story_counts.csv', header=None, index=None)
    ...
    os.system(bq_loc+" query --use_legacy_sql=false 'DELETE FROM `aylien-production`.customer_report.ihs_orange_facts_category WHERE 1=1'")
    os.system(bq_loc+' load --source_format=CSV customer_report.ihs_orange_facts_category ihs_orange_cat_facts.csv')
    
    os.system(bq_loc+" query --use_legacy_sql=false 'DELETE FROM `aylien-production`.customer_report.ihs_orange_facts_language WHERE 1=1'")
    os.system(bq_loc+' load --source_format=CSV customer_report.ihs_orange_facts_language ihs_orange_lang_facts.csv')

    os.system(bq_loc+" query --use_legacy_sql=false 'DELETE FROM `aylien-production`.customer_report.ihs_orange_facts_stories WHERE 1=1'")
    os.system(bq_loc+' load --source_format=CSV customer_report.ihs_orange_facts_stories ihs_orange_story_counts.csv')
